...
public class Solution_5656_SWEA_live {
	...
	static boolean go(int[][] graph, int cnt) {
		...
		for (int c = 0; c < width; c++) {
			...
			if (r == height) { // 피격되는 시작 벽돌이 없는 경우
				...
				copy(graph, newGraph);
				// 제거될 벽돌 연쇄 처리
				boom(newGraph, r, c, newGraph[r][c]);
				// 피격되어 사라진 벽돌 공간에 대한 중력 처리
				down(newGraph);
				// 다음 구슬 던지기
				if (go(newGraph, cnt + 1))
					return true;
			}
	...
	private static void boom(int[][] graph, int r, int c, int cnt) {
